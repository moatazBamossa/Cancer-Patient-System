...
const GLASS_TOKENS: Record<string, GlassToken> = {
  chemotherapy:    { stripe: '#f43f5e', glow: 'rgba(244,63,94,0.22)',    border: 'rgba(244,63,94,0.35)',    dot: 'bg-rose-500',    chip: 'rgba(244,63,94,0.10)' },
  chemo:           { stripe: '#f43f5e', glow: 'rgba(244,63,94,0.22)',    border: 'rgba(244,63,94,0.35)',    dot: 'bg-rose-500',    chip: 'rgba(244,63,94,0.10)' },
  hormonal:        { stripe: '#a855f7', glow: 'rgba(168,85,247,0.22)',   border: 'rgba(168,85,247,0.35)',   dot: 'bg-violet-500',  chip: 'rgba(168,85,247,0.10)' },
  supportive:      { stripe: '#10b981', glow: 'rgba(16,185,129,0.22)',   border: 'rgba(16,185,129,0.35)',   dot: 'bg-emerald-500', chip: 'rgba(16,185,129,0.10)' },
  targeted:        { stripe: '#38bdf8', glow: 'rgba(56,189,248,0.22)',   border: 'rgba(56,189,248,0.35)',   dot: 'bg-sky-400',     chip: 'rgba(56,189,248,0.10)' },
  immunotherapy:   { stripe: '#f59e0b', glow: 'rgba(245,158,11,0.22)',   border: 'rgba(245,158,11,0.35)',   dot: 'bg-amber-500',   chip: 'rgba(245,158,11,0.10)' },
}
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: `linear-gradient(90deg, ${tk.stripe}, transparent)` }}
      />
      ...
      <div className="relative flex flex-col gap-3 p-5">
        ...
        {med.description && (
          <div className="flex items-start gap-1.5">
            <FileText size={11} className="flex-shrink-0 mt-0.5" style={{ color: '#64748b' }} />
...
export default function MedicationsPage() {
  ...
  const [deleteTarget,    setDeleteTarget]    = useState<MedicationRpcItem | null>(null)
  ...
  const { data: medications = [], isLoading } = useQuery({
    ...
    queryFn: () => {
      if (debouncedSearch) return medicationService.searchByName(debouncedSearch)
  ...
  const toOptional = (v?: string) => (v && v.trim() !== '' ? v.trim() : undefined)
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { invalidate(); toast.success(t('common.created')); closeForm() },
  ...
  const updateMut = useMutation({
    ...
    onSuccess: () => { invalidate(); toast.success(t('common.updated')); closeForm() },
    onError:   (err: Error) => toast.error(err.message),
  })
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <Modal
        isOpen={showForm}
        onClose={closeForm}
        title={editItem ? t('medications.editMedication') : t('medications.addMedicationTitle')}
        size="lg"
      >
        <AppForm<MedForm>
          formKey={formKey}
          ...
          onSubmit={(d) => (editItem ? updateMut.mutate(d) : createMut.mutate(d))}
          className="space-y-4"
        >
          {/* Row 1 — Name + Category */}
