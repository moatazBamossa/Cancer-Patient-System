...
export default function CancerTypesPage() {
  ...
  const ctSchema = z.object({
    cancer_name: z
      .string()
      .min(1, t("cancer.nameRequired", { defaultValue: "Name is required" })),
    color: z
      .string()
      .min(1, t("cancer.colorRequired", { defaultValue: "Color is required" })),
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["cancer-types"] })
  ...
  const updateMut = useMutation({
    mutationFn: (d: CTForm) =>
      diagnosisService.updateCancerType(editItem!.cancer_id, d),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["cancer-types"] })
  ...
  const openEdit = (ct: CancerType) => {
    ...
    setFormInitialValues({
      cancer_name: ct.cancer_name,
      color: ct.color,
      icd10_code: ct.icd10_code,
  ...
  const columns: Column<CancerType>[] = [
    ...
    {
      key: "icd10_code",
      header: t("cancer.icd10", { defaultValue: "ICD-10" }),
    },
    {
      key: "description",
  ...
  return (
    ...
    >
      ...
      <Modal
        ...
        title={editItem ? t("cancer.editType") : t("cancer.addTypeTitle")}
        size="sm"
      >
        <AppForm<CTForm>
          formKey={formKey}
        ...
        >
          ...
          <FormField
            name="icd10_code"
            label={t("cancer.icd10", { defaultValue: "ICD-10 Code" })}
          />
          <FormField
            name="description"
            ...
            <Field name="color">
              {({ input }) => (
                <input
                  type="color"
                  {...input}
                  className="w-full h-10 rounded-lg cursor-pointer border p-1 bg-transparent"
          ...
          <div className="flex justify-end pt-4 gap-2">
            <button
              ...
              className="px-4 py-2 rounded-lg text-sm hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
              style={{ color: "var(--text-primary)" }}
            >
            ...
            <button
              ...
              className="gradient-btn px-6 py-2 text-sm rounded-lg disabled:opacity-50"
            >
      ...
      <ConfirmDialog
        ...
        message={t("cancer.deleteConfirm", {
          defaultValue: "Are you sure you want to delete this cancer type?",
        })}
        variant="danger"
      />
    </motion.div>
