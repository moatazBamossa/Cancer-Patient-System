...
export default function TreatmentPlansPage() {
  ...
  const [editCycle, setEditCycle] = useState<TreatmentCycleRpcItem | null>(null)
  ...
  const { data: medications = [], isLoading } = useQuery({
    ...
    queryFn: () => medicationService.listAll(),
  })
  ...
  const handlePlanSubmit = (values: TreatmentPlanFormValues) => {
    const payload = {
      ...
      p_expected_end_date: values.expected_end_date || "",
  ...
  const handleMedSubmit = (values: CycleMedicationFormValues) => {
    const payload = {
      ...
      p_medication_id: Number(values.medication_id),
      p_dose: values.dose,
