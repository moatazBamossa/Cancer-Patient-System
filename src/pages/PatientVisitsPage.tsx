...
import { useClinicVisits, useDeleteVisit, useVisitDetail } from '../hooks/useClinicVisits';
...
export function PatientVisitsPage({ patientId }: PatientVisitsPageProps) {
  ...
  const selectedVisit = selectedVisitDetail ?? visits.find((visit) => visit.visit_id === selectedVisitId) ?? null;
  ...
  const doctorMap = useMemo(
    () => new Map(doctors.map((doctor) => [Number(doctor.doctor_id), doctor.full_name])),
    [doctors],
  );

  const diagnosisMap = useMemo(
    () => new Map(diagnoses.map((diagnosis) => [Number(diagnosis.diagnosis_id), diagnosis])),
    [diagnoses],
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 xl:grid-cols-[380px_1fr]">
        <section className="space-y-4">
          ...
          <div className="rounded-3xl border p-3" style={{ borderColor: 'var(--border-color)', background: 'var(--bg-secondary)' }}>
            {visitsLoading ? (
              <div className="space-y-3">
                {[...Array(4)].map((_, index) => (
                  <div key={index} className="h-24 rounded-2xl bg-slate-200 animate-pulse" />
                ))}
              </div>
            ) : visits.length === 0 ? (
        ...
        <section className="space-y-4">
          ...
          <RecordedVitalSigns visitId={selectedVisit?.visit_id ?? null} onAddVital={() => setIsAddVitalOpen(true)} />
        </section>
      </div>
